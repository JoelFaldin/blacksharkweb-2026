    ...
    <section className="border-y border-(--border) py-20 px-6 bg-(--card)">
      ...
      <div className="grid gap-px md:grid-cols-4">
        {procesos.map((proceso, index) => {
          ...
          return (
            <div key={`process-item-${index}-${proceso.name}`} className={`${style} group flex flex-col items-start justify-start p-10 gap-y-4 transition-colors hover:bg-(--background)`}>
