...
const CartItem = ({ item }: CartItemInterface) => {
  ...
  return (
    <div className="group h-fit max-w-lg md:max-w-2xl flex flex-col gap-6 border border-border bg-card p-4 transition-colors hover:border-primary/30 lg:flex-row lg:items-center md:p-6">
      <div className="flex flex-1 gap-5 items-center">
        ...
        <div className="flex flex-col justify-center gap-1">
          ...
          <p className="line-clamp-2 text-md leading-relaxed text-muted-foreground text-pretty">
            {item.desc}
          </p>
        </div>
      </div>

      <div className="flex flex-row justify-center items-center gap-x-8 gap-y-4">
        <CartQuantity item_id={item.id} cantidad={item.cantidad} />

        <span className="block w-fit text-right">
